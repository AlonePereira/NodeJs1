class LivroDao {
    ...
    buscaPorId(id) {
        return new Promise((resolve, reject) => {
            this._db.get('SELECT * FROM livros WHERE id=?', [id], function(err, resultado){
                if(err) {
                    return reject('Não foi possível buscar o livro pelo id!');
                }
    ...
    remove(id) {
        return new Promise((resolve, reject) => {
            this._db.run('DELETE FROM livros WHERE id=?', [id], function(err){
                if(err) {
